...
function applyEmploymentTemplateData(sheetXml, data) {
  let xml = sheetXml;
  ...
  const industryCell =
    INDUSTRY_CHECKBOX_MAP[String(data.employerIndustry)] ?? INDUSTRY_CHECKBOX_MAP['その他'];
...
function renderEmploymentTemplate(templateBuffer, data) {
  const templateArrayBuffer = toArrayBuffer(templateBuffer);
  const cfb = XLSX.CFB.read(new Uint8Array(templateArrayBuffer), { type: 'array' });

  const fullPaths = cfb.FullPaths || [];
  const index = fullPaths.findIndex((p) => p === 'Root Entry/xl/worksheets/sheet1.xml');
  if (index < 0) {
  ...
  const zipped = XLSX.CFB.write(cfb, { fileType: 'zip', type: 'array' });
  return toArrayBuffer(zipped);
}

module.exports = {
  applyEmploymentTemplateData,
